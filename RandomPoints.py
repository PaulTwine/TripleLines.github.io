...
#fltDistance = np.sqrt(fltArea/(np.pi*intIncrements))
blnStop = False
arrRandom = np.array([(np.random.ranf())*arrBasis[1]+(np.random.ranf())*arrBasis[2]])
...
while (blnStop == False and i < 100000):
    lstPoints = []
    if intPoints < 3:
        arrStart = 0.25*(arrBasis[1]+arrBasis[2])
    elif 3 <= intPoints < 5:
        arrStart = 0.25*arrBasis[1]+0.75*arrBasis[2]
    elif 3 <= intPoints < 5:
        arrStart = 0.75*(arrBasis[1]+arrBasis[2])
    elif 3 <= intPoints < 5:
        arrStart = 0.75*arrBasis[1]+0.25*arrBasis[2]
    else:
        arrStart = np.array([(np.random.ranf())*arrBasis[1]+(np.random.ranf())*arrBasis[2]])
    arrRandom = 0.5*np.array([np.random.beta(2,2)*arrBasis[1]+np.random.beta(2,2)*arrBasis[2]]) +arrStart
    arrRandom = gf.WrapVectorIntoSimulationCell(arrBasis,arrRandom) #-0.5*(arrBasis[1]+arrBasis[2])    
    arrIndices = objPeriodicTree.Pquery_radius(arrRandom, fltDistance)[0]
    if len(np.unique(arrIndices)) == 1 and len(arrIndices[0]) == 0:
        arrRandom = gf.WrapVectorIntoSimulationCell(arrBasis,arrRandom) 
