...
class TJAndGBData(object):
    ...
    def GetMeanGB(self, inCSLGBData: CSLGBData): #returns excess energy weighted by twice the curved grain boundaries to 1 planar oundary
    ...
    def AdjustEnergyByMassBalance(self):
        ...
        arrTJAdjusted = self.__Values[:,3] + arrMu*(arrNMax-self.__Values[:,7])
        arrGBAdjusted = self.__Values[:,2] + arrMu*(arrNMax-self.__Values[:,6])
    ...
    def GetPositionsByDMin(self):
        ...
        for i in self.__DMinValues:
            lstRows = []
            for j in range(10):
                arrRows = np.where((self.__Values[:,1].astype('int') == i) & (self.__Values[:,0].astype('int') == j))[0]
                if len(arrRows) > 0:
...
for strAxis in lstAxes:
    ...
    for intSigma in dctLists[strAxis]:
        ...
        arrCSLValues = np.loadtxt(strBiDir + strAxis + '/' + strBiSigma + 'Values.txt')
        ...
        objTJAndGBData = TJAndGBData(arrAxis,intSigma, arrTJValues,lstDMinValues[0],4)
        dctAllGB[strDMINKey] = objCSL
        dctAllTJ[strDMINKey] = objTJAndGBData
lstMarkers = ['o','v','s']

arrAxes = np.array([np.array([0,0,1]),np.array([1,0,1]),np.array([1,1,1])])
...
for a in dctAllGB.keys():
    ...
    axs[i].scatter(arrSigma, arrGB, c=lstColours[i],marker =lstMarkers[i],label='Small')
...
for a in dctAllTJ.keys():
    ...
    lstTJ.append(arrTJ)
    axs[i].scatter(arrSigma, arrTJ, c=lstColours[i],marker =lstMarkers[i],label='Small')
    axs[i].errorbar(np.mean(arrSigma)-0.25,np.mean(arrTJ),1.96*np.std(arrTJ), c=lstColours[i],linestyle ='',capsize=5,marker='+')
for i in range(3):
    axs[i].set_xticks(list(range(len(lstSigmas[i]))))
    axs[i].set_xticklabels(lstSigmas[i])
...
print(lstWidths)
fig, axs = plt.subplots(1,5,gridspec_kw={'width_ratios':lstWidths}, sharey=True)
for a in dctAllTJ.keys():
    i = np.where(np.all(dctAllTJ[a].GetAxis() == arrAxes,axis=1))[0][0]
    if i ==intAxis:
        arrTJ = dctAllTJ[a].GetTJExcessPerLength()
        ...
        for x in range(len(lstDMins)):
            ...
            axs[axsI].scatter(dCount*np.ones(len(d)), arrTJ[d], c=lstColours[i],marker =lstMarkers[i],label='Small')
...
for a in dctAllGB.keys():
    i = np.where(np.all(dctAllGB[a].GetAxis() == arrAxes,axis=1))[0][0]
    if i ==intAxis:
        lstWidths.append(len(dctAllGB[a].GetDMinValues()))
print(lstWidths)
fig, axs = plt.subplots(1,5,gridspec_kw={'width_ratios':lstWidths}, sharey=True)
for a in dctAllGB.keys():
    i = np.where(np.all(dctAllGB[a].GetAxis() == arrAxes,axis=1))[0][0]
...
for a in dctAllTJ.keys():
    ...
    if i ==intAxis:
        ...
        for x in range(len(lstDMins)):
            d = lstDMins[x]
            strLabel = '.' + str(lstDValue[x])
            plt.scatter(xstart*np.ones(len(d)), arrTJ[d], c=lstColours[i],marker =lstMarkers[i],label='Small')
            xstart += 1
        plt.annotate('$\Sigma$' +str(dctAllTJ[a].GetSigma()), (xstart-(lstHLines[-1]+1)/2,0.08))           
...
for a in dctAllTJ.keys():
    i = np.where(np.all(dctAllTJ[a].GetAxis() == arrAxes,axis=1))[0][0]
    arrTJ = dctAllTJ[a].GetTJExcessPerLength()
    axs[i].hist(arrTJ, color=lstColours[i],bins =np.linspace(-0.4,0.2,24),density = False,stacked=True )
...
plt.hist(arrAllValues, color='gray',bins =np.linspace(-0.4,0.2,24),density = False,stacked=True)
...
for a in dctAllTJ.keys():
    ...
    lstTJs.append(arrTJ)
    lstGBs.append(arrGB)
    plt.scatter(arrGB[arrPositions],dctAllTJ[a].GetTJExcessPerLength()[arrPositions], c=lstColours[i],marker =lstMarkers[i],label='Small') 
plt.axhline(y=0,c='black',linestyle='--')
...
for a in dctAllTJ.keys():
    ...
    plt.scatter(lstGB[-1],lstTJ[-1] , c=lstColours[i],marker =lstMarkers[i],label='Small')
...
for a in dctAllTJ.keys():
    ...
    arrPositions = dctAllTJ[a].GetMinimumTJForEachDMin()
    lstAllTJ.append(gf.FindMediod(dctAllTJ[a].GetTJExcessPerLength()[arrPositions]))
    #arrPositions = np.argmin(dctAllTJ[a].GetExcessEnergy())
    lstAllGB.append(gf.FindMediod(dctAllTJ[a].GetMeanGB(dctAllGB[a])[arrPositions]))
    plt.scatter(lstAllGB[-1], lstAllTJ[-1], c=lstColours[i],marker =lstMarkers[i],label='Small')
...
for a in dctAllTJ.keys():
    ...
    #arrPositions = np.argmin(dctAllTJ[a].GetTJExcessPerLength())
    arrPositions = dctAllTJ[a].GetMinimumTJForEachDMin()
    lstAllTJ.append(np.median(dctAllTJ[a].GetTJExcessPerLength()[arrPositions]))
    #arrPositions = np.argmin(dctAllTJ[a].GetExcessEnergy())
    lstAllGB.append(np.median(dctAllTJ[a].GetMeanGB(dctAllGB[a])[arrPositions]))
    plt.scatter(lstAllGB[-1], lstAllTJ[-1], c=lstColours[i],marker =lstMarkers[i],label='Small')
    plt.annotate(str(dctAllTJ[a].GetSigma()), (lstAllGB[-1], lstAllTJ[-1]))
#lstAllTJ = np.concatenate(lstAllTJ)
#lstAllGB = np.concatenate(lstAllGB)
...
for a in dctAllTJ.keys():
    ...
    print(dctAllTJ[a].GetAxis(),dctAllTJ[a].GetSigma(),dctAllTJ[a].GetValues()[intMin,:2],dctAllTJ[a].GetValues()[intMax,:2])
