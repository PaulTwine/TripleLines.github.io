...
# %%
def FindRationalQR(inMatrix: np.array):
    intSize = np.shape(inMatrix)[1]
    arrMatrix = np.copy(inMatrix)
    lstH = []
    lstS = []
    lstM = []
    i = 0
    while i < intSize:
        arrUnitVector = np.ones(i)
        arrUnitVector[0] = 1
        arrRow = arrMatrix[i,i:] 
        fltS = np.dot(arrRow,arrRow)
        arrH = np.identity(intSize -i) - 2*np.outer(arrRow, arrRow)/fltS
        arrHFull = np.identity(intSize)
        arrHFull[i:,i:] = arrH
        lstH.append(arrHFull)
        lstS.append(fltS)
        i +=1
        arrMatrix = np.matmul(arrHFull,arrMatrix)
    arrDiagonal = np.diag([1/lstS[0],1/lstS[1],1/lstS[2]])
    return arrMatrix
# %%
...
#arrMatrix = FindRationalQR(2*arrCellBasis)
...
pts = gf.MergePeriodicClusters(pts,objTJ.GetCellVectors(),['p','p','p'])
plt.axis('equal')
