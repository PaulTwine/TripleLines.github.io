...
class CSLConjugateBases(object):
    ...
    def FindOrthogonalReciprocalVectors(self,inCSLBasis):
        lstTransforms = []
        arrR = np.transpose(gf.FindReciprocalVectors(np.transpose(2*inCSLBasis)))
        lstVectors = []
        intL = np.round(np.linalg.det(inCSLBasis)/0.25,0).astype('int')
        self.__SigmaValue = abs(intL)
        #lstPermutations = list(it.permutations(list(range(-abs(intL),abs(intL))),3))
        #lstVectors = list(map(lambda x: x[0]*arrR[0]+x[1]*arrR[1]*x[2]*arrR[2],lstPermutations))
        intL1 = np.ceil(1/np.abs(np.dot(arrR[0],gf.NormaliseVector(np.cross(arrR[1],arrR[2]))))).astype('int')+1
        intL2 = np.ceil(1/np.abs(np.dot(arrR[1],gf.NormaliseVector(np.cross(arrR[2],arrR[0]))))).astype('int')+1
        intL3 = np.ceil(1/np.abs(np.dot(arrR[2],gf.NormaliseVector(np.cross(arrR[0],arrR[1]))))).astype('int')+1
        ...
        for n in arrRows2:
            ...
            arrRows3,arrCols3 = np.where(np.abs(np.matmul(arrVectors2,np.transpose(arrVectors2))) < 1e-10)
...
#%%            
objCSL = gl.CSLTripleLine(np.array([7,7,1]), ld.FCCCell)
arrCell = objCSL.FindTripleLineSigmaValues(200)
intIndex = np.where(np.all(arrCell[:,:,0].astype('int')==[27,27,81],axis=1))[0][0]
...
# objSigma = gl.SigmaCell(np.array([4,1,1]),ld.FCCCell)
...
lstNewT = objCSLConjugate.FindOrthogonalReciprocalVectors(arrCellBasis)
