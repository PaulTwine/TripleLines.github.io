...
#         arrCentre = 0.5*arrCellVectors[0]+0.25*arrCellVectors[1]
#         arrCentre[:2]
#         # for o in objLT.GetGrainLabels():
#         #     if o == 0:
#         #         pts = objLT.GetAtomsByID(objLT.GetGrainAtomIDs(o))[:,1:4]
#         #         ax.scatter(pts[:,0], pts[:,1], pts[:,2], s=0.1)
#         # plt.show()
#         # ids = objLT.PartitionNonGrainSegments()
#         # pts = objLT.GetAtomsByID(ids)[:,1:4]
#         # ax.scatter(pts[:,0], pts[:,1], pts[:,2], s=0.1)
#         # plt.show()
        
       
#         for k in objLT.GetGrainLabels():
#             if k > 0:
#                 lstNewGrainNumbers.append(k)
#                 lstNewGrainIDs.append(objLT.GetGrainAtomIDs(k).tolist())
#         if len(lstOldGrainIDs) > 0:
#             lstMatched = mf.MatchPairsOfIDs(lstOldGrainIDs, lstNewGrainIDs)
#         lstOldGrainIDs = lstNewGrainIDs
#         lstNewGrainIDs = []
#         lstNewGrainNumbers = []
...
#             arrFinalMeans = np.array(list(map(lambda x: np.mean(x[:2],axis=0), lstMerged)))
#             arrDisplacements =  arrFinalMeans - arrCentre
#             intIndex = np.argmin(np.linalg.norm(arrDisplacements,axis=1))
#             if len(lstPositions) == 0:
#                 lstPositions.append(arrDisplacements[intIndex][1]/deltaT)
#             else:
#                 lstPositions.append((arrDisplacements[intIndex][1]-lstPositions[-1])/deltaT)
#                 deltaT = 1
#         else:
#             deltaT += 1
#             print('non-matching grains ' , objLT.GetGrainLabels(), k)
#     lstDisplacements.append(np.mean(list(map(lambda x: np.linalg.norm(x),lstPositions))))
# lstLogV = list(map(lambda x: np.log(x),lstDisplacements))
...
for i in lstTemp:
    ...
    arrRealIndices = objInitialTree.GetPeriodicIndices(arrIndices)
    arrFinalMeans = arrFinalMeans[np.ravel(arrRealIndices),:]
    ...
    for k in range(0,100000+dt,dt):
        if objLT.GetGrainLabels() == [0,1,2,3]:   
            ...
            #intMin = np.argmin(np.linalg.norm(arrMeanPoints-0.5*(arrCellVectors[0]+arrCellVectors[1]) ,axis=1))
            ...
            arrDistancesF, arrIndicesF = objFinalTree.Pquery(arrMeanPoints,1)
            arrPositionsF = np.where(arrDistancesF < 1)[0]
            arrDistancesI, arrIndicesI = objInitialTree.Pquery(arrMeanPoints,1)
            arrPositionsI = np.where(arrDistancesF > 1)[0]
            # if len(arrPositionsI) > 0 :
            #     lstKeys = list(dctInitial.keys())
            #     for a in arrPositionsI:
            #         if a not in lstKeys:
            #             dctInitial[a] = (arrDistancesI[a],k)
            # if len(arrPositionsF) > 0 :
            #     lstKeys = list(dctFinal.keys())
            #     for a in arrPositionsF:
            #         if a not in lstKeys:
            #             dctFinal[a] = (arrDistancesF[a],k)
...
for p in range(len(lstProjections)):
    plt.scatter(lstAllTimes[p],lstProjections[p][0], c='red')
