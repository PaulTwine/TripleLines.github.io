...
print(hermite_normal_form(m, domain=ZZ))
Matrix([[1, 0, 0], [0, 10, 0], [0, 0, -30]])
#%%
C = hermite_normal_form(np.array([[1,1],[2,2]]))
print(C)
...
#%%
def DSCPoints(lstOfPoints:list, ):
    intL = len(lstOfPoints)
    lstAllPoints = []
    for i in range(intL):
        for j in range(1,intL):
            for p in lstAllPoints[i]:
                lstAllPoints.extend(lstOfPoints[j]+p)
                lstAllPoints.extend(lstOfPoints[j]-p)
    return lstOfPoints
...
plt.axis('off')
plt.scatter(pts[:,0],pts[:,1],c='darkred')
plt.show()
...
plt.gca().set_aspect('equal')
plt.axis('off')
plt.scatter(pts2[:,0],pts2[:,1],c='darkblue')
plt.show()
#%%
fig = plt.figure()
ax = fig.add_subplot(projection='3d')
